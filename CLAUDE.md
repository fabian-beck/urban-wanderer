...
# Evaluation
npm run eval:stories -- --lat <lat> --lon <lon> [--vary ...] [--design random --sample n --seed n] [--retries n] [--segments n] [--dry-run]
npm run eval:judge -- [--run <dir>] [--repeats n] [--unit segment|chain] [--with-context] [--report-only]
```

## Headless Evaluation

`scripts/story-personalization-eval.mjs` generates stories for one location under a full-factorial set of preference variations (guide character, familiarity, labels, language, model) and runs the app's own code in Node: `updateLocation(coords, { background: false })` and `loadMetadata({ loadImages: false })` from `src/stores.js` build the place context, `generateStory` from `src/util/ai-story.js` produces the segments. A file-backed `localStorage` stand-in in `.eval-cache/` keeps the app caches across runs. The script prints the number of configurations, story requests and pipeline runs before asking for confirmation, then generates story chains in parallel. Results go to `eval-results/stories/<timestamp>/` (`results.jsonl`, `stories.md`, `contexts/`, `run.json`). `--context <file>` reuses a saved place context to isolate the story prompt effect. `--design random --sample n --seed n` replaces the full factorial with a balanced random sample (each level equally often, level pairs spread evenly by seeded random swaps), a near-orthogonal design for main effects. Both output directories are gitignored.

`scripts/story-personalization-judge.mjs` rates the generated texts blind (the judge never sees preferences or configuration) with `AI_EVAL_JUDGE_MODEL` from `src/constants/ui-config.js` via structured output: language difficulty, language formality, depth and breadth of information on a 1–5 scale, plus topic focus as a multi-selection of the preference labels. It aggregates means, SDs and topic rates per varied factor level and configuration, computes topic precision and recall against the preferred labels, and writes `judgments.jsonl`, `judgments-summary.json` and a self-contained `report.html` (inline SVG charts, light and dark theme, no external resources) into the run directory. With two or more varied factors it also fits an additive least-squares model and reports each level's adjusted (estimated marginal) mean and the model's R², which corrects the raw means for the other factors in sampled designs. `--repeats n` measures judge consistency, `--report-only` rebuilds the report without new API calls.
